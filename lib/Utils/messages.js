            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        ...
                        serverMessageId: 0,
                        newsletterName: "OURIN INFORMATION",
                        contentType: "UPDATE",
                    }
                }
            ]
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        for (const card of message.cards) {
            let headerMsg;
            if (card.image) {
                headerMsg = await (0, exports.prepareWAMessageMedia)({ image: card.image }, options);
            } else if (card.video) {
                headerMsg = await (0, exports.prepareWAMessageMedia)({ video: card.video }, options);
            }

            const header = headerMsg
                ? { hasMediaAttachment: true, ...(headerMsg.imageMessage ? { imageMessage: headerMsg.imageMessage } : { videoMessage: headerMsg.videoMessage }) }
                : undefined;
            ...
            cards.push({
                ...
                nativeFlowMessage: card.buttons?.length
                    ? { buttons: card.buttons, messageParamsJson: '' }
                    : undefined,
            });
        }
        ...
        m.viewOnceMessage = {
            message: {
                interactiveMessage: {
                    header: message.title ? { title: message.title, subtitle: message.subtitle || '', hasMediaAttachment: false } : undefined,
        ...
        const coverUpload = await (0, exports.prepareWAMessageMedia)(
            { image: coverBuffer },
